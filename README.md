...
## 本次更新内容
- 修正条码与格口规则：条码解析改为固定规则“`02-格口号`=拆零、`Z-格口号`=整件”，并新增 `BarcodeParseResult.TargetChuteCode` 输出；请求格口服务改为基于条码解析目标格口（仍保留任务命中与已扫描状态校验）；同步更新 `条码规则基线.md` 与 `对外API接口基线.md`。
...
- `条码规则基线.md`：固化条码输入约束、固定匹配规则（`02-格口号`/`Z-格口号`）、解析输出与失败语义，约束扫描链路判定口径。
...
- `Application/Abstractions/Services/IBarcodeParser.cs` + `Application/Services/BarcodeParser.cs`：条码解析服务抽象与实现，按固定规则 `02-格口号`（拆零）与 `Z-格口号`（整件）分类，统一输出 `TargetChuteCode` 与失败语义（InvalidBarcode、UnsupportedBarcodeType、ParseError）。
