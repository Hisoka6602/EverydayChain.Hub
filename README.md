...
## 本次更新内容
- `sync_target_state` 表按 `TableCode` 分表：每个同步表编码对应独立的状态表（`sync_target_state_{tableCode}`），避免所有表状态堆积在同一张表；建表时自动使用包含 TableCode 的唯一约束名，防止 SQL Server 命名冲突；`GetSyncStateTableFullName` 改为 `internal static` 以支持测试直接断言。
...
- 新增4个测试：`GetSyncStateTableFullName_ShouldGeneratePerTableCodeName`（Theory×3路径）、`GetSyncStateTableFullName_WhenTableCodeContainsInvalidChar_ShouldThrow`，覆盖状态分表命名安全边界。
...
- `SqlServerSyncUpsertRepository.cs`：SQL Server 真实落库实现，按目标逻辑表+后缀分表执行集合式 MERGE（支持配置回退逐行模式），并在 `sync_target_state_{tableCode}` 分表（按 TableCode 分表，每张同步表独立一张状态表）中记录后缀用于跨分表迁移更新时的旧分表批量清理。
...
- `EverydayChain.Hub.Tests/Repositories/SqlServerSyncUpsertRepositoryTests.cs`：SQL Server 落库仓储契约测试，覆盖插入/更新/跳过统计、UniqueKeys 缺失异常，以及 sync_target_state 状态分表命名安全边界（正常路径×3 + 非法字符抛异常）。
