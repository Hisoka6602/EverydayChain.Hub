...
- 文档新增高效与准实时策略：增量窗口、分页批量、低往返读取、可配置误差窗口（如 10 分钟）。
...
- `Oracle到SQLServer同步架构设计.md`：定义外部 Oracle DB First 只读同步到本地 SQL Server 的详细落地方案，包含接口/实现命名、配置模型、幂等覆盖、删除同步、分表保留期治理与验收清单。
