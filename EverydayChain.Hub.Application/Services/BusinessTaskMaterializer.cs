...
public class BusinessTaskMaterializer : IBusinessTaskMaterializer
{
    ...
    public BusinessTaskEntity Materialize(BusinessTaskMaterializeRequest request)
    {
        ...
        var materializedTimeLocal = ValidateMaterializedTimeLocal(request.MaterializedTimeLocal);
    ...
    private static DateTime ValidateMaterializedTimeLocal(DateTime? materializedTimeLocal)
    {
        if (!materializedTimeLocal.HasValue || materializedTimeLocal.Value == default)
        {
            throw new InvalidOperationException("MaterializedTimeLocal 缺失，无法确定业务时间，禁止继续写入业务任务。");
        }
