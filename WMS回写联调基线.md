...
## 2. 当前启用策略
- 当前策略：生产默认关闭，联调配置齐全。
- 本次上线最终口径：当前版本可上线，但 WMS 回写保持关闭（`WmsFeedback.Enabled=false`）。
...
## 11. 本次上线最终结论（收口）
### 11.1 方案 A（本次采用）
- 结论：当前版本可直接上线，但 WMS 回写关闭。
- 配置要求：生产环境保持 `WmsFeedback.Enabled=false`。
- 影响说明：WMS 回写不作为本次上线阻塞项，不影响主系统 API、查询与报表能力上线。

### 11.2 方案 B（后续启用）
- 仅当“第 10 节生产启用门禁”全部通过后，才允许将 `WmsFeedback.Enabled` 调整为 `true`。
- 在门禁未全部通过前，禁止以“先上线后补审”方式在生产启用 WMS 回写。
