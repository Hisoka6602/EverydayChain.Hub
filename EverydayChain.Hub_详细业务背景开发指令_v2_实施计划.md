...
| PR-13 | 里程碑M1全量审查-基础建模阶段 | PR-01~PR-03 | 新增 0 / 修改 2~4 / 合计 2~4 | `feature/pr13-m1-full-review` |
| PR-14 | 里程碑M2全量审查-主链路打通阶段 | PR-04~PR-07 | 新增 0 / 修改 2~4 / 合计 2~4 | `feature/pr14-m2-full-review` |
...
| PR-17 | 里程碑M5全量审查-联调收口阶段 | PR-12 | 新增 0 / 修改 2~4 / 合计 2~4 | `feature/pr17-m5-full-review` |
...
2. 执行全仓构建与测试并记录结果。
3. 将问题按阻塞级别归档到台账。
4. 输出基础建模阶段阻塞问题清单并形成后续修复计划。
5. 完成里程碑 M1 审查结论归档并补充待确认项清单。
...
4. 输出主链路打通阶段阻塞问题清单并形成后续修复计划。
5. 完成里程碑 M2 审查结论归档并补充待确认项清单。
...
2. 复核审计字段完整性与一致性。
3. 执行全仓构建与测试并记录结果。
4. 输出回传与审计阶段阻塞问题清单并形成后续修复计划。
5. 完成里程碑 M3 审查结论归档并补充待确认项清单。
...
2. 复核重试幂等、上限和审计记录完整性。
3. 执行全仓构建与测试并记录结果。
4. 输出异常与补偿阶段阻塞问题清单并形成后续修复计划。
5. 完成里程碑 M4 审查结论归档并补充待确认项清单。
