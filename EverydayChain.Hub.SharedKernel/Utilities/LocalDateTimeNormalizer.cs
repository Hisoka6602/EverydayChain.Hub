...
public static class LocalDateTimeNormalizer {
    ...
    /// <param name="utcRejectedMessage">当输入为 UTC 时返回的错误消息。</param>
    /// <param name="normalizedTime">规范化后的时间。</param>
    /// <param name="validationMessage">校验失败消息。</param>
    /// <returns>校验是否通过。</returns>
    public static bool TryNormalize(DateTime candidateTime, string utcRejectedMessage, out DateTime normalizedTime, out string validationMessage) {
        if (candidateTime.Kind == DateTimeKind.Utc) {
            normalizedTime = default;
            validationMessage = utcRejectedMessage;
            return false;
        }
